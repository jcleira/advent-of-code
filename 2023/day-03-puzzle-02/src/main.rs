    ...
    for (x, row) in matrix.iter().enumerate() {
        for (y, item) in row.iter().enumerate() {
            if is_symbol(*item) {
                ...
                sum += multiply_adjacent_numbers(&matrix, matrix_tracker, x, y);
                println!("sum {}", sum);
            }
        }
    }
    ...
    for i in x_min..x_max + 1 {
        for j in y_min..y_max + 1 {
            if matrix[i][j].is_numeric() {
                let number = build_number(&matrix, matrix_tracker, i, j);
                if number == 0 {
                    continue;
                }
                ...
                println!("found number {}", found_numbers[found_numbers.len() - 1]);
            }
        }
    }
